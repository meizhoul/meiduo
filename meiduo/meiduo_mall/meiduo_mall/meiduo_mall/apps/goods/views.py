...
#             return http.HttpResponseForbidden("categor_id不存在")
#
#         #接收前端传入的sort=“xxxx”
...
#
#         #创建分页器paginator(要进行分页的所有数据，指定每页显示多少数据)
...
#             category = GoodsCategory.objects.get(id=category_id)
#         except GoodsCategory.DoesNotExist:
#             return http.HttpResponseForbidden("catagory_id不存在")
#
#         #查询出当前指定三级类别中销量最高的前两个商品
...
#
#         #now_date = timezone.localdate()#获取当前时间日期
#         now_date = timezone.now() #获取当前时间对象
#
#         try:
